...
import { diaYMes, esFechaValida, nombreDia, sumarDias } from "@/components/paciente/fechas";
import {
  IconoCamara,
  IconoCerrar,
  IconoGaleria,
...
// y el tipo se re-sugiere con cada hora nueva mientras el paciente no lo elija a mano.

type Cuando = {
  fecha: string;
  hora: string;
  tipo: TipoComida;
  tipoElegido: boolean; // el paciente tocó un chip: no lo pisamos más
...
export function FormularioComida({ pacienteId, hoy, valores, comida }: FormularioComidaProps) {
  ...
  async function guardar(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (foto?.origen === "nueva") {
        setFase("subiendo");
        const path = rutaFoto(pacienteId);
      ...
      // paciente_id no se manda: lo pone la base (default auth.uid()).
      ...
      setFase("listo");
      startTransition(() => router.replace(`/paciente?fecha=${fecha}`));
    } catch (err) {
  ...
  async function eliminar() {
    ...
    setFase("listo");
    startTransition(() => router.replace(`/paciente?fecha=${valores.fecha}`));
  }
        ...
        <fieldset disabled={ocupado} className="flex min-w-0 flex-col gap-6">
          ...
          <label className="block">
            ...
            <textarea
              ...
              placeholder="¿Qué comiste? Ej: 2 tostadas con queso y café con leche"
        ...
        <div className="mt-2 pb-[calc(1.5rem_+_env(safe-area-inset-bottom))]">
          {confirmandoBorrado ? (
            <div className="rounded-2xl border border-borde bg-peligro-suave p-4">
              <p className="font-medium text-peligro">¿Eliminar esta comida?</p>
...
/** Borrado "de mejor esfuerzo": si falla queda un archivo huérfano, pero no frenamos al paciente. */
