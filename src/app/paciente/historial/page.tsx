...
import { diaYMes, nombreDia, sumarDias } from "@/components/paciente/fechas";
import { IconoDerecha } from "@/components/paciente/iconos";
  ...
  return (
    <div className="pb-6">
      ...
      <p className="mt-1 text-sm text-tinta-suave">
        Cargaste comidas en {diasConComidas} de los últimos {DIAS} días.
      </p>
        ...
        {dias.map(({ fecha, cantidad, faltan }) => (
          <li key={fecha}>
            ...
            >
              ...
              >
                {cantidad}
                <span className="sr-only">{cantidad === 1 ? " comida" : " comidas"}</span>
              </span>
...
function Estado({
  ...
  if (cantidad === 0) {
    return fecha === hoy ? (
      ...
      <p className="text-sm text-acento">Sin comidas cargadas</p>
    );
  }
  if (faltan.length === 0) {
    return (
